%% BENG 227 MIDTERM PROJECT Evan Masutani, Group1
% DESCRIPTION: Code for our improved model and comparison with original
% model
%% HOUSEKEEPING
clear all;
close all;
...
%% INITIAL CONDITIONS; refer to icy.m for usage and https://github.com/allardjun/WavingCrawling/tree/master/src
...
%% RUN WITH GRADIENT I.C.: DIFFUSION (Our improved model)
%% SIMULATION RUN PARAMETERS
% step sizes
dr = 1;
% Declare an isotropic spacer, dx
...
A(:,1) = AMaxODE * ones(N_r,1);
M(:,1) = zeros(N_r,1);
% Initial conditions to stimulate central movement of lamellopodia
...
% %% PLOT LEADING EDGE DISPLACEMENT: Commented out to prevent breaking subplot, but will work if you comment back in!
% figure
% for frame = 1:1000:5000
%     hold on
%     plot(dista(:,frame),cell(:,2),'LineWidth',10)
...
%     ylim([0 N_r]);
% end
% legend('0 sec','500 sec','1000 sec','1500 sec','2000 sec','2500 sec')
% figure
%%% CODE TO GENERATE VIDEOS, COMMENT OUT IF NO VIDS WANTED
% figure
...
% end
% close(video1);
%% RUN WITH GRADIENT I.C.: NO DIFFUSION (Original model)
% RUN WITH BOUNDARY CONDITIONS; targeted plots
delta = 0.8;
R = 0.12;
B = noise * rand(N_r,N_t);
